...
function cycleImages(){
      ...
      //image
       ...
       $next = ($active.next().length > 0) ? $active.next() : $('#cycler img:first');
	  //thumbs
    ...
    $(window).load(function(){
	  ...
	  $('#slider_thumbs .half_black').click(function(){
				...
				container.find('img').css('z-index',1);
				
				//set the clicked image to the top
				container.find('img.active').removeClass('active').css('z-index',1);
				container.find('img').eq($(this).attr('rel')).css('z-index',4).addClass('active');
