<?php
	...
	if(isset($_POST['pic_uploaded']))
	{
		if($_POST['pic_uploaded'])
		{
			...
			$query=mysql_query("INSERT INTO images (user_id, food_id, image_type) VALUES('$userid', '$food_id','2')");
			
			$image_id=mysql_insert_id();
			
			$query=mysql_query("INSERT INTO activity (activity_id, user_id, food_id,date, image_id) VALUES('7','$userid','$food_id','".strtotime("now")."' , '$image_id' )");
			...
			$query=mysql_query("UPDATE userinfo SET badges='$new_badges', points=points+".ADD_PIC_POINTS.", num_pictures=num_pictures+1  WHERE user_id='$userid'");
			
			$img_extension=substr($_POST['pic_uploaded'],strpos($_POST['pic_uploaded'],'.') );
			
			$img_filename="food_pic".$image_id.$img_extension;
         ...
         // echo $end_chars1;     
				...
				$query =  mysql_query("select dish_image from food_items where food_items_id ='$food_id' limit 1");
				$food_prev_img = mysql_fetch_array($query);
				$prev_img = $food_prev_img["dish_image"];
			...
			header("location: food.php?eid=".$food_id);
		}
		else
		{
			/* Form was incomplete. */
...
menu, nav, section { display: block; }
   ...
   <form enctype="multipart/form-data" id="food_form" action="<?php echo $_SERVER['PHP_SELF'].'?fid='.$food_id;?>" METHOD="POST">
					
					<div>
						<div>
							<?php
								
								$query = mysql_query( "select dish_name, rest_id from food_items where food_items_id = '$food_id'");
							...
							?>
							<a href="<?php echo get_page_url("food",array("eid"=>$food_id));?>">
							<img src="<?php echo get_page_url("food_pic",array("fid"=>$food_id, "size"=>"thumb")); ?>" style="margin-left:5px; float:left;"/></a>
							<br>
							
								<div class="food_item" style="padding-bottom:8px; margin-left:10px; float:left;">
										<h1 class="food_name" style="font-size:24px;"><a href="<?php echo get_page_url("food",array("eid"=>$food_id));?>"><?php echo $food['dish_name']; ?></a><br><font class="text_orange">@<a href="<?php echo get_page_url("restaurant",array("eid"=>$food["rest_id"]));?>" style="color:inherit;"> <?php echo $restr['res_name']; ?></a></font></h1>
								</div>
						</div>
