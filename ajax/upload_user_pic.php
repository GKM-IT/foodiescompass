<?php
	...
	define('IMG_RESIZE_WIDTH',160);
	define('IMG_RESIZE_HEIGHT',160);
	...
	if($_FILES['food_pic']['error']==UPLOAD_ERR_OK && $_FILES['food_pic']['size']<=MAX_UPLOAD_SIZE && in_array($_FILES['food_pic']['type'],$mime))
	{
		...
		require_once(BASE_DIR."include/resize_img.inc.php");
				...
				$end_chars1 = strtolower(substr($temp_filename, -4));
				$end_chars2 = strtolower(substr($temp_filename, -5));
         // echo $end_chars1;     
		...
		$resizer->save2(BASE_DIR.UPLOAD_DIR.$temp_filename, $image);
	...
	else if($_FILES['food_pic']['error']==0)
	{
		...
		else if(!in_array($_FILES['food_pic']['type'],$mime))
		{
			...
			$json="{'status':'9','msg':'".$_FILES['food_pic']['tmp_name']."'}";
			echo $json;
		}
