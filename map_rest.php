...
if(!$user_lat) { $user_lat = $_POST["food_lat"];$user_lng =$_POST["food_lng"];$user_place = $_POST["food_place"];}
		...
		$query="SELECT * from restaurant where rsID ='$food' ";
	//echo $query;
	...
	if($food["res_lat"]) { 
	//$distance = distance( (double)$food["res_lat"], (double)$food["res_lng"], $user_lat, $user_lng, "k" );
...
<div style="background:#faaf42; padding:20px; color:#fff; font-weight:bold; font-size:18px;">
								...
								echo 'Your Location is not set. <br>Select Your Location';
								else
								echo 'You @ '.$user_place;
								echo '</div>
...
location.reload();
			...
			var mapOptions = {';
			   if($user_lat!='false' || $user_lat=='')
			echo '  center: new google.maps.LatLng('.$user_lat.','.$user_lng.'),';
			else
			echo '  center: new google.maps.LatLng('.$food["res_lat"].','.$food["res_lng"].'),';
			echo'  zoom: 12,
